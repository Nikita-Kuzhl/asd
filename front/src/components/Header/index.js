        ...
        <Container>
          ...
          <Navbar.Collapse className="gap-4 ms-5 fs-5">
            ...
            <div className="ms-auto d-flex gap-3">
              {auth.user ? (
                <>
                  <Button onClick={()=>navigate('/cabinet')} variant="light">Личный кабинет</Button>
                  <Button onClick={()=>navigate('/cart')} variant="light">Корзина</Button>
                  <Button variant="light" onClick={() => auth.delUser()}>
                    Выход
                  </Button>
                </>
              ) : (
